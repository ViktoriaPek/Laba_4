package bsu.rfe.java.group10.lab4.Pekarskaya.varA4;
...
// Перо для рисования графика
        graphicsStroke = new BasicStroke(5.0f, BasicStroke.CAP_BUTT,
                BasicStroke.JOIN_ROUND, 10.0f, new float[] {7, 14, 9, 12, 7, 9, 14, 9, 12, 9, 12}, 0.0f);
...
сегментов (GeneralPath) * Начало пути устанавливается в первую точку графика, после чего
...
/*public boolean ResearchMarkers(double x){
        int nech= (int) x;
        if(nech%2!=0)
            return true;
        else false;
...
// Шаг 2 - Организовать цикл по всем точкам графика
        for (Double[] point : graphicsData) {
            if(ResearchMarkers(point[1]==true)){
    ...
    protected  void  paintMarkers ( Graphics2D canvas ) {
        ...
        for ( Double [] point : graphicsData) {
           ...
           Path. moveTo (center.getX(), center . getY ());
            Path. lineTo (Path. getCurrentPoint (). getX (), Path . getCurrentPoint (). getY () + 10);
            Path. lineTo (Path . getCurrentPoint () . getX ()+10, Path . getCurrentPoint () . getY () -10);
            Path. moveTo (center . getX (), center . getY ());
           // Path. lineTo (center . getX () -  10 , center . getY ()+10);
           Path. lineTo (Path . getCurrentPoint () . getX ()-10, Path . getCurrentPoint () . getY () );
            Path . lineTo (Path . getCurrentPoint () . getX ()+10, Path . getCurrentPoint () . getY () + 10 );
            Path . moveTo (center . getX (), center . getY ());
            //Path . lineTo (center . getX ()+10, center . getY () -  10 );
            Path . lineTo (Path . getCurrentPoint () . getX () + 10 , Path . getCurrentPoint () . getY ());
...
// Вывести надпись в точке с вычисленными координатами
            ...
            Point2D.Double labelPosMin = xyToPoint(-0.2, minY);
            String str1 = Double.toString(minY);
            canvas.drawString(str1, (float) labelPosMin.getX() + 10,
                    (float) labelPosMin.getY());

            Point2D.Double labelPosMax = xyToPoint(-0.2, maxY);
            String str2 = Double.toString(maxY);
            canvas.drawString(str2, (float) labelPosMax.getX() + 10,
                    (float) 50);
        }
